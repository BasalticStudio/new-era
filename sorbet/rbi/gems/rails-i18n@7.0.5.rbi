# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for types exported from the `rails-i18n` gem.
# Please instead update this file by running `bin/tapioca gem rails-i18n`.

# Used for Czech, Slovak.
#
# source://rails-i18n-7.0.5/lib/rails_i18n/railtie.rb:3
module RailsI18n; end

# source://rails-i18n-7.0.5/lib/rails_i18n/railtie.rb:4
class RailsI18n::Railtie < ::Rails::Railtie
  class << self
    # source://rails-i18n-7.0.5/lib/rails_i18n/railtie.rb:20
    def add(pattern); end

    # source://rails-i18n-7.0.5/lib/rails_i18n/railtie.rb:30
    def init_pluralization_module; end

    # source://rails-i18n-7.0.5/lib/rails_i18n/railtie.rb:25
    def pattern_from(args); end
  end
end
